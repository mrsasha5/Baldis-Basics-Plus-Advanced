...
namespace BaldisBasicsPlusAdvanced
{
    ...
    public class AdvancedCore : BaseUnityPlugin
    {
        ...
        {
            /*byte[] encodedBytes;
            using (FileStream fs = new FileStream("build.zip", FileMode.Open, FileAccess.Read))
            {
                encodedBytes = RijndaelEncryption.Encrypt(fs.ToByteArray(), "");
            }

            using (FileStream encodedFs = new FileStream("e_build.zip", FileMode.OpenOrCreate, FileAccess.Write))
            {
                encodedFs.Write(encodedBytes, 0, encodedBytes.Length);
            }*/
