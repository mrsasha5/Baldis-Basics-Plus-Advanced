...
{
    public class ControllerSystemBase : MonoBehaviour  
    {
        ...
        private void Update()
        {
            ...
            for (int i = 0; i < controllers.Count; i++)
            {
                ...
                {
                    ControllerBase controller = controllers[i];
        ...
        public bool CreateController<T>() where T : ControllerBase, new()
        {
            T controller = new T();
        ...
        public bool CreateController<T>(out T controller) where T : ControllerBase, new()
        {
            T _controller = new T();

            controller = _controller;

            if (_controller.MaxCount == -1 || GetAbsoluteControllersCount<T>() < _controller.MaxCount)
            {
        ...
        public void RemoveController<T>(T controller) where T : ControllerBase
        {
            controllers.Remove(controller);
        }

        public int GetControllersCount<T>() where T : ControllerBase
        {
            return controllers.FindAll(x => x.GetType() == typeof(T) && !x.ToDestroy).Count; 
        }

        public int GetAbsoluteControllersCount<T>() where T : ControllerBase
        {
            return controllers.FindAll(x => x.GetType() == typeof(T)).Count;
        }

        protected virtual T InitController<T>(T controller) where T : ControllerBase
        {
            return controller;
        }
