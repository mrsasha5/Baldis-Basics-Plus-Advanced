...
{
    public class ExpelHammerPickup : PickupBase
    {
        ...
        private void InitializeMenu(int player)
        {
            ...
            chalkboardMenu.GetText("title").text = Singleton<LocalizationManager>.Instance.GetLocalizedText("ItemAdv_ExpelHammer");
            chalkboardMenu.GetText("info").text = string.Format(
                Singleton<LocalizationManager>.Instance.GetLocalizedText("ItemAdv_ExpelHammer_ReturnPurchase"),
                    boughtHammerPrice
                );
