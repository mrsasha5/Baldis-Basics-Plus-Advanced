...
namespace BaldisBasicsPlusAdvanced.Game.Builders
{
    ...
    public class BaseStructure_Plate : StructureBuilder, IPrefab
    {
        ...
        public override void PostOpenCalcGenerate(LevelGenerator lg, System.Random rng)
        {
            ...
            if (includeHalls)
            {
                AdvancedCore.Logging.LogInfo($"{name} is building plates on PostOpenCalcGenerate(LevelGenerator, System.Random) in the halls.");
        ...
        public virtual BasePlate BuildPrefab(BasePlate platePre, Cell cell, Direction dir)
        {
            ...
            AdvancedCore.Logging.LogInfo($"{name} is placing {platePre.name} at {cell.position.ToString()} with {dir.ToString()} direction in the room {cell.room.name}.");
            return plate;
        }
