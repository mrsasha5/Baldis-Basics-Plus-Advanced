...
#warning TODO: invent calculation algorithm to avoid plates accumulation (brrr)
        protected virtual List<Cell> GetCells()
        {
            ...
            /*if (avoidCloseTiles)
            {
                List<Cell> cells = new List<Cell>(tilesOfShape);
                ...
                for (int i = 0; i < cells.Count; i++)
                {
                    ...
                    {
                        weight += Vector3.Distance(cells[i].FloorWorldPosition, cells[j].FloorWorldPosition);
                    }
                    weights.Add(weight);
                }
                ...
                for (int i = 0; i < cells.Count; i++)
                {
                    ...
                    {
                        weight += Vector3.Distance(cells[i].FloorWorldPosition, plates[j].transform.position);
                    }
                    weights[i] += weight;
                }
                ...
                while (cells.Count > 0)
                {
                    int index = weights.IndexOf(MathHelpher.FindMaxValue(weights));
                    tilesOfShape.Add(cells[index]);
        ...
        public virtual void Build(LevelGenerator lg, System.Random rng, bool isRoomCells)
        {
        ...
        public override void OnGenerationFinished(LevelGenerator lg)
        {
            ...
            if (includeRooms) Build(lg, lg.controlledRNG, isRoomCells: true);
