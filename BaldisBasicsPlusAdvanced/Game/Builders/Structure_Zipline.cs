...
namespace BaldisBasicsPlusAdvanced.Game.Builders
{
    public class Structure_Zipline : StructureBuilder, IPrefab
    {
        ...
        public override void OnGenerationFinished(LevelGenerator lg)
        {
            ...
            if (lengths.Count > 0)
            {
                int max = MathHelpher.FindMaxValue(lengths.ToArray());

                int index = lengths.IndexOf(max);

                Build(lg, straightPaths[index]);
            }
        }

        public void Build(LevelGenerator lg, KeyValuePair<Cell, Cell> cells)
        {
            ...
            ZiplineHanger hanger = Instantiate(WeightedGameObject.ControlledRandomSelection(parameters.prefab, lg.controlledRNG).GetComponent<ZiplineHanger>());
            hanger.transform.parent = gm.transform;
            hanger.Initialize(ec, lg.controlledRNG, new KeyValuePair<Vector3, Vector3>(cells.Key.TileTransform.position - direction * hanger.Offset,
