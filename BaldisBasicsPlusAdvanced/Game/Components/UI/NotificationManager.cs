...
namespace BaldisBasicsPlusAdvanced.Game.Components.UI
{
    public class NotificationManager : Singleton<NotificationManager>
    {
        ...
        public void Queue(string key, SoundObject sound = null, float time = 10f, bool isForced = false)
        {
            if (!AdvancedCore.notificationsEnabled && !isForced) return;
            gameObject.SetActive(true);
            notifsQueue.Enqueue(new Notification()
            {
                key = key,
                sound = sound,
                time = time
        ...
        private IEnumerator NotificationAnimator(bool appearing)
        {
            ...
            if (appearing)
            {
                while (pos.y < maxHeight)
                {
                    ...
                    pos.y += Time.deltaTime * currentNotif.speed;
