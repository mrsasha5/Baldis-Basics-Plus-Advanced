        ...
        public static List<NPC> GetPotentialCharacters()
        {
            ...
            for (int i = 0; i < npcs.Count; i++)
            {
                ...
                if (!meta.tags.Contains("adv_exclusion_hammer_weakness")
                    && (ld.forcedNpcs.Contains(npc)
                    || Array.Find(ld.potentialBaldis, x => x.selection.Character == npc.Character) != null))
                    continue;

                if (LevelDataManager.LevelData.bannedCharacters.Contains(meta.character)) continue;
