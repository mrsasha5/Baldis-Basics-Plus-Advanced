...
namespace BaldisBasicsPlusAdvanced.Patches
{
    public static class AssetsExtensions
    {
        ...
        public static void SetCellsRange(this LevelAsset level, IntVector2 vector1, IntVector2 vector2, int roomId, bool buildWallsAround = true)
        {
            int x1 = vector1.x;
            int z1 = vector1.z;
            while (x1 < vector2.x)
            {
                while (z1 < vector2.z)
                {
                    IntVector2 pos = new IntVector2(x1, z1);
                    CellData cellData = new CellData()
                    {
                        pos = pos,
                        roomId = roomId
