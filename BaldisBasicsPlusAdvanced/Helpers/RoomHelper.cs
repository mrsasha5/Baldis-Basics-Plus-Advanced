...
namespace BaldisBasicsPlusAdvanced.Helpers
{
    public class RoomHelper
    {
		...
		public static List<RoomAsset> CreateAssetsFromPath(string path, int maxItemValue, bool isOffLimits, RoomFunctionContainer existingContainer = null, bool isAHallway = false, bool isASecretRoom = false, Texture2D mapBg = null, bool keepTextures = true, bool squaredShape = false)
        {
            ...
            List<RoomAsset> assets = new List<RoomAsset>();
        ...
        internal static List<RoomAsset> GetAssetsFromLevelAsset(LevelAsset lvlAsset, string roomname, int minRoomRange, int maxRoomRange, int maxItemValue, bool isOffLimits, RoomFunctionContainer existingContainer = null, bool isASecretRoom = false, Texture2D mapBg = null, bool keepTextures = true, bool squaredShape = false)
        {
            ...
            for (int idx = minRoomRange; idx < maxRoomRange; idx++)
            {
                ...
                IntVector2 posOffset = new IntVector2(lvlAsset.levelSize.x, lvlAsset.levelSize.z);
                ...
                Vector3 worldPosOffset = new Vector3(posOffset.x * 10f, 0f, posOffset.z * 10f);
                ...
                {
                    IntVector2 pos = IntVector2.GetGridPosition(rAsset.basicObjects[i].position);
                ...
                rAsset.name = $"Room_{rAsset.category}_{roomname}" + (maxRoomRange >= 2 ? string.Empty : idx.ToString());
        ...
        public static RoomAsset CreateAssetFromPath(string path, bool isOffLimits, bool autoAssignFunctionContainer, RoomFunctionContainer existingContainer = null, bool isAHallway = false, bool isASecretRoom = false, Texture2D mapBg = null, bool keepTextures = true, bool squaredShape = false)
        {
            ...
            using (BinaryReader reader = new BinaryReader(File.OpenRead(path)))
            {
                ...
                rAsset.forcedDoorPositions = new List<IntVector2>(lvlAsset.rooms[idx].forcedDoorPositions);
                rAsset.hasActivity = lvlAsset.rooms[idx].hasActivity;
                rAsset.itemList = new List<ItemObject>(lvlAsset.rooms[idx].itemList);
                rAsset.items = new List<ItemData>(lvlAsset.rooms[idx].items);
                for (int i = 0; i < rAsset.basicObjects.Count; i++)
                {
