...
namespace BaldisBasicsPlusAdvanced.Helpers
{
    public class RoomHelper
    {
        ...
        public static RoomAsset CreateAssetFromPath(string path, bool isOffLimits, RoomFunctionContainer existingContainer = null, bool isAHallway = false, bool isASecretRoom = false, Texture2D mapBg = null, bool keepTextures = true, bool squaredShape = false)
        {
        ...
        public static RoomAsset CreateAssetFromPath(string path, Transform lightPre, bool isOffLimits, RoomFunctionContainer existingContainer = null, bool isAHallway = false, bool isASecretRoom = false, Texture2D mapBg = null, bool keepTextures = true)
        {
            ...
            using (BinaryReader reader = new BinaryReader(File.OpenRead(path)))
            {
                ...
                {
                    RoomFunctionContainer roomFunctionContainer = new GameObject(rAsset.name + "FunctionContainer").AddComponent<RoomFunctionContainer>();
                    //roomFunctionContainer.functions = []; // For some freaking reason, this is not initialized by default
                    ReflectionHelper.setValue<List<RoomFunction>>(roomFunctionContainer, "functions", new List<RoomFunction>());
                    roomFunctionContainer.gameObject.ConvertToPrefab(true);
                ...
                GameObject.Destroy(lvlAsset); // Remove the created level asset from memory
            }
            return rAsset;
        }*/
