...
namespace BaldisBasicsPlusAdvanced.Helpers
{
    public class PrefabsCreator
    {
        ...
        public static T SetupPrefableComponentOnPrefab<T>(GameObject obj, int variant = 1, bool putInMemory = true) where T : MonoBehaviour, IPrefab
        {
            T prefabComponent = obj.AddComponent<T>();
            prefabComponent.InitializePrefab(variant);

            if (putInMemory) ObjectsStorage.Objects.Add(obj.name, obj);
